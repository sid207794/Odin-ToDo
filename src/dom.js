...
const myListDialog = (function () {
  ...
  function listNameEventListener() {
    ...
    listName.style.width =
      (mirror.offsetWidth / window.innerWidth) * 100 + "vw";
    listName.addEventListener("input", () => {
      ...
      if (list) {
        ...
        const item = document.querySelector(
          `#sidebar .myList .${CSS.escape(listUID)}`
        );
      ...
      if (detail) {
        ...
        const item = document.querySelector(
          `#content .${CSS.escape(detailUID)}.detail .buttons .listName`
        );
...
function contentCreate(UID) {
  ...
  function addTask(UID) {
    const submit = document.querySelector(
      `#content .${CSS.escape(UID)} .addTask`
    );
    const footer = document.querySelector(
      `#content .${CSS.escape(UID)} .footerInputBox`
    );
    const input = document.querySelector(
      `#content .${CSS.escape(UID)} #footerInput`
    );
    const timeInput = document.querySelector(
      `#content .${CSS.escape(UID)} #dateTimePicker`
    );
    const today = document.querySelector(`#content .${CSS.escape(UID)} .today`);
    const tomorrow = document.querySelector(
      `#content .${CSS.escape(UID)} .tomorrow`
    );
    const upcoming = document.querySelector(
      `#content .${CSS.escape(UID)} .upcoming`
    );
    const viewText = document.querySelector("#left .view span");
    ...
    submit.addEventListener("click", () => {
      ...
      if (input.value !== "") {
        ...
        if (timeInput.value === "" || date === currentDate) {
          ...
          const weekDayName = weekDay.toLocaleDateString("en-US", {
            weekday: "short",
          });
          submitConditionals(
            `${weekDayName} ${time}`,
            tomorrow,
            targetClass.tomorrow,
            null,
            weekDayName,
            time
          );
        ...
        // function for submit event listener conditionals
        function submitConditionals(
          spanText,
          day,
          targetClassDay,
          dateValue,
          ...
          requestAnimationFrame(() => {
            if (day.children.length === 2 || day.querySelector(".fadeIn")) {
              newTask.classList.add("fadeIn");
              newTask.style.display = "block";
            } else if (!day.querySelector(".fadeIn")) {
          ...
          newTask.addEventListener("click", (e) => {
            const content = document.querySelector("#content");
            const detail = document.querySelector("#content .detail");
            const contentDetail = document.querySelector(
              `#content .${CSS.escape(taskUID)}.detail`
            );
            if (!checkbox.contains(e.target)) {
              ...
              const allTasks =
                document.querySelectorAll(`#content .list .task`);
                ...
                if (content.contains(detail)) {
                  ...
                  setTimeout(() => {
                    if (content.contains(detail)) {
                      ...
                      detailCard(taskUID);
                      const inputTask = document.querySelector(
                        "#content .detail .title #taskName"
                      );
                      inputTask.style.height = "auto";
                      inputTask.style.height =
                        (inputTask.scrollHeight / window.innerHeight) * 100 +
                        "vh";
                    }
                  }, 200);
                } else {
                  ...
                  inputTask.style.height = "auto";
                  inputTask.style.height =
                    (inputTask.scrollHeight / window.innerHeight) * 100 + "vh";
                }
          ...
          checkbox.addEventListener("change", () => {
            const completeImg = document.querySelector(
              `#content .${CSS.escape(taskUID)}.detail .header .complete`
            );
            Object.entries(targetClassDay).forEach(([key, task]) => {
              if (
                typeof task === "object" &&
                task.id === taskUID &&
                checkbox.checked
                ...
                if (completeImg) {
                  ...
                  checkImg.style.filter =
                    "invert(46%) sepia(96%) saturate(1918%) hue-rotate(185deg) brightness(101%) contrast(102%)";
                }
                localStorage.setItem("todoData", JSON.stringify(listArray));
              } else if (
                typeof task === "object" &&
    ...
    function populateExisitingList() {
      if (
        Object.keys(targetClass.today).some((key) => key.startsWith("task"))
      ) {
        populateListConditionals(today, targetClass.today);
      }

      if (
        Object.keys(targetClass.tomorrow).some((key) => key.startsWith("task"))
      ) {
        populateListConditionals(tomorrow, targetClass.tomorrow);
      }

      if (
        Object.keys(targetClass.upcoming).some((key) => key.startsWith("task"))
      ) {
        populateListConditionals(upcoming, targetClass.upcoming);
      }

      // Function for populateExistingList conditinoals
      function populateListConditionals(day, targetClassDay) {
        Object.keys(targetClassDay).forEach((key) => {
          if (key.startsWith("task")) {
            const task = targetClassDay[key];
            const newTask = document.createElement("div");
            const label = document.createElement("label");
            const checkbox = document.createElement("input");
      ...
      if (
        listToday.children.length +
          listTomorrow.children.length +
          listUpcoming.children.length >=
        11
      ) {
        const h3 = document.querySelectorAll("#content .list h3");
        h3.forEach((itemh3) => {
  ...
  function detailCard(taskUID) {
    const task = document.querySelector(
      `#content .list .${CSS.escape(taskUID)} label`
    );
    const span = document.querySelector(
      `#content .list .${CSS.escape(taskUID)} label span`
    );
    const checkbox = document.querySelector(
      `#content .list .${CSS.escape(taskUID)} label input#${CSS.escape(
        taskUID
      )}`
    );
    const taskTextNode = Array.from(task.childNodes).find(
      (node) => node.nodeType === Node.TEXT_NODE
    );
    const targetClass = listArray.find((item) => item.ListId === UID);
    ...
    inputTask.addEventListener("input", () => {
      ...
      Object.keys(targetClass).forEach((key) =>
        Object.entries(targetClass[key]).forEach(([taskKey, taskObj]) => {
    ...
    inputNote.addEventListener("input", () => {
      ...
      inputNote.style.height =
        (inputNote.scrollHeight / window.innerHeight) * 100 + "vh";
    });
    ...
    const completeImg = document.querySelector(
      "#content .detail .header .complete"
    );
    ...
    inputAttach.addEventListener("change", (e) => {
      ...
      const fileName = inputAttach.files[0]
        ? inputAttach.files[0].name
        : "Choose File";
      const p = document.querySelector(
        `#content .${CSS.escape(taskUID)}.detail .attachments label p`
      );
    ...
    headDelete.addEventListener("click", () => {
      ...
      const listToday = document.querySelectorAll(
        "#content .list .today .task"
      );
      const listTomorrow = document.querySelectorAll(
        "#content .list .tomorrow .task"
      );
      const listUpcoming = document.querySelectorAll(
...
const burgerDialog = (function () {
  ...
  burger.addEventListener("click", (event) => {
    ...
    if (dialog.style.display === "none") {
      ...
      dialog.style.left = `${
        ((burger.offsetLeft - 46) / window.innerWidth) * 100
      }vw`;
      burger.style.backgroundColor = "#161616";
      burgerImg.style.filter =
        "invert(46%) sepia(96%) saturate(1918%) hue-rotate(185deg) brightness(101%) contrast(102%)";
    } else {
      dialog.style.opacity = "1";
      dialog.style.transform = "scale(1)";
...
function deleteAction(UID) {
  ...
  deleteListBtn.addEventListener("click", () => {
    const deleteDialog = document.querySelector(
      `.listDeleteConfirm.${CSS.escape(UID)}`
    );
    const deleteDialogDiv = document.querySelector(
      `.listDeleteConfirmDiv.${CSS.escape(UID)}`
    );
    const deleteYes = document.querySelector(
      `.listDeleteConfirmDiv.${CSS.escape(UID)} .yes.${CSS.escape(UID)}`
    );
    const deleteNo = document.querySelector(
      `.listDeleteConfirmDiv.${CSS.escape(UID)} .no.${CSS.escape(UID)}`
    );
